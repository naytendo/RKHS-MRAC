...
[offlineError,PowerBound,condNum,norm_fs] = getOfflineErrorNewt(Ns,type,par,scale,OmegaLim,targetNu,targetType,targetScale,targetCoefs,interps,Dim,Newt,ext);

model = 'NewtonMRAC';
open(model)
for tt = 1:length(runTimes)
fillDist = 2*OmegaLim./Ns;
ssErrorWendland = zeros(length(Ns),1);
approxError = zeros(length(Ns),1);
condNum = zeros(length(Ns),1);
condNumKL = zeros(length(Ns),1);
cs = getActiveConfigSet(model);
csNew = copy(cs);
set_param(csNew,"StopTime",num2str(runTimes(tt)))
StartMeasureTime = runTimes(tt)-2;


for nn = 1:length(Ns)
N = Ns(nn);

epsilon = PowerBound(nn)+delta;
...
hold on
lineSS = loglog(Ns,ssErrorWendland/B,'-d','linewidth',1.5);
